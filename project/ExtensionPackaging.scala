...
object ExtensionPackaging extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] =
    Seq(
      ...
      // To write files to Aeneas NODE directory
  ...
  private def maintainerFix =
    inConfig(Linux)(
      Seq(
        maintainer := "aeneas.pm",
        packageSummary := s"Aeneas node ${name.value}${network.value.packageSuffix} extension",
        packageDescription := s"Aeneas node ${name.value}${network.value.packageSuffix} extension"
      ))

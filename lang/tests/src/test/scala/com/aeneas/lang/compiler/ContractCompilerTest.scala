...
import com.aeneas.lang.Common.{NoShrink, produce}
...
import com.aeneas.lang.v1.compiler.{CompilerContext, Terms}
import com.aeneas.lang.v1.evaluator.FunctionIds
import com.aeneas.lang.v1.evaluator.ctx.impl.waves.{FieldNames, Types, WavesContext}
...
class ContractCompilerTest extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  property("hodlContract") {
    val ctx = Monoid
      .combineAll(
        Seq(
          PureContext.build(com.aeneas.lang.Global, V3).withEnvironment[Environment],
          CryptoContext.build(com.aeneas.lang.Global, V3).withEnvironment[Environment],
          WavesContext.build(
            DirectiveSet(V3, Account, DAppType).explicitGet()
          )
        ))
  ...
  property("wavesBalanceV4 have type BalanceDetails") {
    val ctx = Monoid
      .combineAll(
        Seq(
          PureContext.build(com.aeneas.lang.Global, V4).withEnvironment[Environment],
          CryptoContext.build(com.aeneas.lang.Global, V4).withEnvironment[Environment],
          WavesContext.build(

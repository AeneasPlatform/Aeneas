...
class TransferTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime {
  ...
  test("asset transfer changes sender's and recipient's asset balance by transfer amount and ash by fee") {
  ...
  test("ash transfer changes ash balances and eff.b. by transfer amount and fee") {
    for (v <- transferTxSupportedVersions) {
      val firstBalance     = sender.wavesBalance(firstAddress).available
      val firstEffBalance  = sender.wavesBalance(firstAddress).effective
      val secondBalance    = sender.wavesBalance(secondAddress).available
  ...
  test("invalid signed ash transfer should not be in UTX or blockchain") {
  ...
  test("can not make transfer without having enough ash balance") {
    for (v <- transferTxSupportedVersions) {
      val firstBalance     = sender.wavesBalance(firstAddress).available
      val firstEffBalance  = sender.wavesBalance(firstAddress).effective
      val secondBalance    = sender.wavesBalance(secondAddress).available

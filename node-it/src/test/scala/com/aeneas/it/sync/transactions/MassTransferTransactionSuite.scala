...
import com.aeneas.api.http.requests.{MassTransferRequest, SignedMassTransferRequest}
import com.aeneas.common.state.ByteStr
import com.aeneas.common.utils.EitherExt2
import com.aeneas.it.api.MassTransferTransactionInfo
...
import com.aeneas.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
...
class MassTransferTransactionSuite extends BaseTransactionSuite /*with CancelAfterFailure*/ {
  ...
  test("Ash mass transfer changes ash balances") {
    for (v <- massTransferTxSupportedVersions) {
      val (balance1, eff1) = miner.accountBalances(firstAddress)
      val (balance2, eff2) = miner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough ash") {
    for (v <- massTransferTxSupportedVersions) {
      val (balance1, eff1) = miner.accountBalances(firstAddress)
      val (balance2, eff2) = miner.accountBalances(secondAddress)

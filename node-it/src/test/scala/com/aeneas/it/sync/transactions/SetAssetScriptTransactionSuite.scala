...
import com.aeneas.api.http.ApiError.{CustomValidationError, Mistiming, StateCheckFailed, WrongJson}
import com.aeneas.common.state.ByteStr
import com.aeneas.common.utils.{Base58, EitherExt2}
import com.aeneas.crypto
import com.aeneas.it.api.SyncHttpApi._
import com.aeneas.it.sync.{script, someAssetAmount, _}
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("sender's ash balance is decreased by fee") {
  ...
  test("cannot transact without having enough ash") {
    val (balance, eff) = miner.accountBalances(firstAddress)
    for (v <- setAssetScrTxSupportedVersions) {

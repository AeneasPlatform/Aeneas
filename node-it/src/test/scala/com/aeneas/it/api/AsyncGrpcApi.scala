...
import com.aeneas.account.{AddressScheme, Alias, KeyPair}
...
import com.aeneas.protobuf.transaction.{PBOrders, PBSignedTransaction, PBTransactions}
...
object AsyncGrpcApi {
  implicit class NodeAsyncGrpcApi(val n: Node) {
    ...
    def stateChanges(
    ...
    ): Future[Seq[(com.aeneas.transaction.Transaction, StateChangesDetails)]] = {
      ...
      result.runToFuture.map { r =>
        import com.aeneas.state.{InvokeScriptResult => VISR}
        r.map { r =>

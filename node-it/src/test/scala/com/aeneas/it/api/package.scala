...
import com.aeneas.utils.{Paged, ScorexLogging}
...
package object api {
  ...
  implicit val addressReads: Reads[com.aeneas.account.Address] = Reads {
    case JsString(addrStr) =>
      com.aeneas.account.Address
        .fromString(addrStr)
        .fold(err => JsError(err.toString), addr => JsSuccess(addr))
    case _ => JsError("Expected base58 encoded address")
  }

  implicit val dstMapReads: Reads[Map[com.aeneas.account.Address, Long]] = Reads { json =>
    json.validate[Map[String, Long]].map { dst =>
      dst.map {
        case (addrStr, balance) =>
          com.aeneas.account.Address.fromString(addrStr).explicitGet() -> balance
      }
    }
  }
  ...
  implicit val distributionReads: Reads[AssetDistribution] = Reads { json =>
    json
      .validate[Map[com.aeneas.account.Address, Long]]
  ...
  implicit val distributionPageReads: Reads[AssetDistributionPage] = Reads { json =>
    json.validate[Paged[com.aeneas.account.Address, AssetDistribution]].map(pg => AssetDistributionPage(pg))
  }
}

...
import com.aeneas.api.http.ApiError
import com.aeneas.api.http.RewardApiRoute.RewardStatus
import com.aeneas.api.http.requests.IssueRequest
import com.aeneas.common.state.ByteStr
import com.aeneas.common.utils.EitherExt2
import com.aeneas.features.api.{ActivationStatus, FeatureActivationStatus}
import com.aeneas.http.DebugMessage
...
import com.aeneas.state.{AssetDistribution, AssetDistributionPage, DataEntry, Portfolio}
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    import com.aeneas.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.aeneas.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollbackWithoutBlacklisting(height: Int, returnToUTX: Boolean = true): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.aeneas.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def rollbackToBlockId(id: String): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.aeneas.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollbackToBlockId(id)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Int): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.aeneas.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }

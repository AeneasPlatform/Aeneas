package com.aeneas.it.account.storage
...
import com.aeneas.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, CONST_STRING}
...
class RemoveEntrySuite extends BaseSuite {
  ...
  "Remove entry from account storage" - {
    for (data <- Seq(stringTestData, integerTestData, booleanTestData, binaryTestData)) s"${data.ct}Entry could be removed from account storage" in {
      ...
      miner.getDataByKey(address, data.k).getClass.getCanonicalName shouldBe s"com.aeneas.state.${data.ct}DataEntry"

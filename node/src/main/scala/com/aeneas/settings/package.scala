...
package object settings {
  ...
  def loadConfig(maybeUserConfig: Option[Config]): Config = {
    ...
    val cmdDefaults =
      Try(withApp.getConfig("node.defaults"))
        .getOrElse(ConfigFactory.empty())
        .atPath("node")

    external
      .withFallback(cmdDefaults)
      .withFallback(ConfigFactory.parseString(s"node.directory = ${defaultDirectory(withApp)}"))
      .withFallback(ConfigFactory.defaultApplication())
      .withFallback(ConfigFactory.defaultReference())
      .resolve()
  }
  ...
  def defaultDirectory(config: Config): String = {
    ...
    def withNetwork(config: Config): String = {
      val bc = config.getString("node.blockchain.type")
      val suffix =
        if (bc == "CUSTOM") {
          val char = config.getString("node.blockchain.custom.address-scheme-character").headOption.getOrElse(0.toChar)

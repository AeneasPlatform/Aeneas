...
import com.aeneas.block.{Block, MicroBlock, SignedBlockHeader}
import com.aeneas.common.state.ByteStr
import com.aeneas.database.Storage
import com.aeneas.events.BlockchainUpdateTriggers
import com.aeneas.features.BlockchainFeatures
import com.aeneas.lang.ValidationError
import com.aeneas.metrics.{TxsInBlockchainStats, _}
import com.aeneas.mining.{Miner, MiningConstraint, MiningConstraints}
import com.aeneas.settings.{BlockchainSettings, WavesSettings}
import com.aeneas.state.diffs.BlockDiffer
import com.aeneas.state.reader.{CompositeBlockchain, LeaseDetails}
import com.aeneas.transaction.Asset.{IssuedAsset, Waves}
import com.aeneas.transaction.TxValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
...
import com.aeneas.utils.{ScorexLogging, Time, UnsupportedFeature, forceStopApplication}
import kamon.Kamon
...
class BlockchainUpdaterImpl(
  ...
  override def processBlock(block: Block, hitSource: ByteStr, verify: Boolean = true): Either[ValidationError, Seq[Diff]] =
    writeLock {
      ...
      val notImplementedFeatures: Set[Short] = leveldb.activatedFeaturesAt(height).filter(_ != 100).diff(BlockchainFeatures.implemented)
      println(s"heightL${leveldb.height}")
      println (displayFeatures(leveldb.activatedFeaturesAt(height)))
      println (displayFeatures(BlockchainFeatures.implemented))
      Either
        .cond(
...
object BlockchainUpdaterImpl {
  ...
  private def displayFeatures(s: Set[Short]): String =
    s"FEATURE${if (s.size > 1) "S" else ""} ${s.map(x => s"${BlockchainFeatures.feature(x)} == $x").mkString(", ")} ${if (s.size > 1) "have been" else "has been"}"

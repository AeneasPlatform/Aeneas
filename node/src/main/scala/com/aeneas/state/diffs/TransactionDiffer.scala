...
import com.aeneas.features.BlockchainFeatures.BlockV5
import com.aeneas.lang.ValidationError
import com.aeneas.metrics.TxProcessingStats
import com.aeneas.metrics.TxProcessingStats.TxTimerExt
import com.aeneas.state.InvokeScriptResult.ErrorMessage
import com.aeneas.state.diffs.invoke.InvokeScriptTransactionDiff
import com.aeneas.state.{Blockchain, Diff, InvokeScriptResult, LeaseBalance, NewTransactionInfo, Portfolio, Sponsorship}
...
import com.aeneas.transaction.smart.{InvokeScriptTransaction, SetScriptTransaction, Verifier}
    ...
    if (verify)
      stats.commonValidation
        .measureForType(tx.typeId) {
          for {
            _ <- {
              //println(s"tx addr:${tx.}")
              CommonValidation.disallowFromAnotherNetwork(tx, AddressScheme.current.chainId)
            }

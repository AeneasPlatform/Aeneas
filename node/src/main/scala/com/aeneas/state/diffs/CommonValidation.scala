...
import cats._
import com.aeneas.account.{Address, AddressScheme, DefaultAddressScheme}
import com.aeneas.features.OverdraftValidationProvider._
...
object CommonValidation {
  ...
  def disallowFromAnotherNetwork[T <: Transaction](tx: T, currentChainId: Byte): Either[ValidationError, T] =
    Either.cond(
      ...
      tx,
      GenericError(
        s"Data from other network: expected:${DefaultAddressScheme.chainId} ${AddressScheme.current.chainId}(${AddressScheme.current.chainId.toChar}), actual: ${tx.chainId}(${tx.chainId.toChar})"
      )
    )

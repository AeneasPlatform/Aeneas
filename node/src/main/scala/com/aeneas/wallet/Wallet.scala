...
import com.aeneas.account.{Address, KeyPair, PrivateKey}
import com.aeneas.common.state.ByteStr
import com.aeneas.common.utils.Base58
import com.aeneas.crypto
import com.aeneas.lang.ValidationError
import com.aeneas.settings.WalletSettings
import com.aeneas.transaction.TxValidationError.MissingSenderPrivateKey
import com.aeneas.utils.{JsonFileStorage, randomBytes, _}
...
object Wallet extends ScorexLogging {
  ...
  private[this] final class WalletImpl(maybeFile: Option[File], passwordOpt: Option[String], maybeSeedFromConfig: Option[ByteStr])
    ...
    def generateNewSecret(): Array[Byte] = {
      val prevSecrets = maybeSeedFromConfig
      val nonce: Array[Byte] = Ints.toByteArray(prevSecrets.size)
      val s = Blake2b256(seed ++ nonce)
      val (priv, _) = crypto.createKeyPair(seed)
      val allSecrets: TreeSet[ByteStr] = TreeSet(ByteStr(priv)) ++ prevSecrets

      allSecrets.toArray.flatMap(p => p.arr)
    }


    implicit def orderingByOrder[A <: PrivateKey]: Ordering[A] =
      Ordering.by(e => e.base64)


    private[this] lazy val actualSeed: ByteStr = maybeSeedFromConfig
    .getOrElse {
    ...
    private[this] def generateNewAccountWithoutSave(nonce: Int): Option[KeyPair] = WalletLock.write {
      ...
      if (!accountsCache.contains(address)) {
        ...
        walletData = walletData.copy(accountSeeds = walletData.accountSeeds + ByteStr(account.seed))
        log.info(s"Added account #${privateKeyAccounts.size};address : ${account.toAddress.stringRepr}")
        Some(account)
      } else None
    }

...
object PBTransactions {
  ...
  def create(
      sender: com.aeneas.account.PublicKey,
      chainId: Byte = 0,
      fee: Long = 0L,
  ...
  def toPBDataEntry(de: com.aeneas.state.DataEntry[_]): DataTransactionData.DataEntry = {
  ...
  def toVanillaScript(script: ByteString): Option[com.aeneas.lang.script.Script] = {
    import com.aeneas.common.utils._
    if (script.isEmpty) None else Some(ScriptReader.fromBytes(script.toByteArray).explicitGet())
  }

  def toPBScript(script: Option[com.aeneas.lang.script.Script]): ByteString = script match {

...
import com.aeneas.account.{KeyPair, PrivateKey, PublicKey}
import com.aeneas.common.state.ByteStr
import com.aeneas.common.utils.{Base58, Base64, FastBase58}
...
object UtilApp {
  ...
  private[this] object Actions {
    ...
    def doSign(c: Command, data: Array[Byte]): ActionResult =
      Right(com.aeneas.crypto.sign(c.signOptions.privateKey, data).arr)

    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.aeneas.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
    ...
    def doHash(c: Command, data: Array[Byte]): ActionResult = c.hashOptions.mode match {
      case "fast"   => Right(com.aeneas.crypto.fastHash(data))
      case "secure" => Right(com.aeneas.crypto.secureHash(data))

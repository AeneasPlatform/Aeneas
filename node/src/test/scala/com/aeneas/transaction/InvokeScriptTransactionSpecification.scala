...
import com.aeneas.api.http.requests.{InvokeScriptRequest, SignedInvokeScriptRequest}
import com.aeneas.common.state.ByteStr
import com.aeneas.common.utils.{Base64, _}
...
import com.aeneas.protobuf.transaction.{PBAmounts, PBRecipients, PBTransaction, PBTransactions}
import com.aeneas.transaction.smart.{InvokeScriptTransaction, Verifier}
import com.aeneas.{EitherMatchers, TransactionGen, crypto}
...
class InvokeScriptTransactionSpecification extends PropSpec with PropertyChecks with Matchers with EitherMatchers with TransactionGen {
  ...
  property("protobuf roundtrip") {
    forAll(invokeScriptGen(paymentListGen), accountGen) { (tx, caller) =>
      val unsigned = com.aeneas.protobuf.transaction.PBTransaction(
        tx.chainId,
      ...
      val proof        = crypto.sign(caller.privateKey, PBTransactions.vanilla(com.aeneas.protobuf.transaction.PBSignedTransaction(Some(unsigned))).explicitGet().bodyBytes())
      val signed       = com.aeneas.protobuf.transaction.PBSignedTransaction(Some(unsigned), Seq(ByteString.copyFrom(proof.arr)))
  ...
  property(s"can't have more than ${ContractLimits.MaxInvokeScriptArgs} args") {
    import com.aeneas.common.state.diffs.ProduceError._

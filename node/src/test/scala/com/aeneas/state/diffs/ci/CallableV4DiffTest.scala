...
import com.aeneas.lang.directives.values.{Asset, V4}
...
import com.aeneas.state.diffs.FeeValidation.{FeeConstants, FeeUnit}
import com.aeneas.state.diffs.TransactionDiffer.TransactionValidationError
import com.aeneas.state.diffs.{ENOUGH_AMT, _}
import com.aeneas.state.{EmptyDataEntry, SponsorshipValue}
...
import com.aeneas.transaction.{GenesisTransaction, Transaction, TxVersion}
    ...
    forAll(multiActionPreconditions(feeMultiplier = 5, withScriptError = false)) {
      case (genesis, setScript, invoke, issue, _, _, _, _, _) =>
        assertDiffEi(
          ...
          TestBlock.create(Seq(invoke)),
          features
        )(_ should produce(s" with 6 total scripts invoked does not exceed minimal value of $minimalFee ASH"))
    }
  }

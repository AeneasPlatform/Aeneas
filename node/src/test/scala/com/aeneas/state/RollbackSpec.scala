...
import com.aeneas.lang.v1.{FunctionHeader, compiler}
import com.aeneas.settings.{TestFunctionalitySettings, WavesSettings}
import com.aeneas.state.reader.LeaseDetails
import com.aeneas.transaction.Asset.{IssuedAsset, Waves}
import com.aeneas.transaction.TxValidationError.AliasDoesNotExist
import com.aeneas.transaction.assets.{IssueTransaction, ReissueTransaction}
...
import com.aeneas.transaction.{CreateAliasTransaction, DataTransaction, GenesisTransaction, Transaction, TxVersion}
import com.aeneas.utils.StringBytes
import com.aeneas.{NoShrink, TestTime, TransactionGen, history}
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender.toAddress, com.aeneas.state.diffs.ENOUGH_AMT))
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender.toAddress, com.aeneas.state.diffs.ENOUGH_AMT))
...
object RollbackSpec {
  private val issueReissueBurnScript = {
    import com.aeneas.lang.directives.values.{DApp => DAppType}

...
case class Domain(db: DB, blockchainUpdater: BlockchainUpdaterImpl, levelDBWriter: LevelDBWriter) {
  ...
  def appendMicroBlock(txs: Transaction*): Unit = {
    ...
    val signature = com.aeneas.crypto.sign(defaultSigner.privateKey, block.bodyBytes())
  ...
  def createBlock(version: Byte, txs: Seq[Transaction]): Block = {
    ...
    Block
      .buildAndSign(
        ...
        generationSignature = com.aeneas.history.generationSignature,
        txs = txs,
        featureVotes = Nil,
        rewardVote = -1L,

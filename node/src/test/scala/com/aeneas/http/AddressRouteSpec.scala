...
import com.aeneas.account.{Address, AddressOrAlias}
import com.aeneas.api.common.CommonAccountsApi
import com.aeneas.api.http.AddressApiRoute
...
import com.aeneas.lang.contract.DApp.{CallableAnnotation, CallableFunction, VerifierAnnotation, VerifierFunction}
...
import com.aeneas.{NoShrink, TestTime, TestWallet, WithDB, crypto}
...
class AddressRouteSpec
  ...
  routePath(s"/scriptInfo/${allAddresses(1)}") in {
    ...
    Get(routePath(s"/scriptInfo/${allAddresses(3)}")) ~> route ~> check {
      ...
      // [WAIT]                                           Decompiler(
      //      testContract,
      //      Monoid.combineAll(Seq(PureContext.build(com.aeneas.lang.directives.values.StdLibVersion.V3), CryptoContext.build(Global))).decompilerContext)
